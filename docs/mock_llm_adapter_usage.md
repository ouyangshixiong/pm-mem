...
MockLLM适配器提供了与DeepSeek API兼容的测试用模拟LLM接口。Mock 只应在测试或本地实验中显式启用；生产路径应使用真实 DeepSeek 配置，失败时直接报错。

## 主要特性

1. **完全兼容性**: 与DeepSeekClient接口完全兼容
2. **显式模式**: 测试时显式启用 Mock，真实调用时必须提供 DeepSeek API Key
...
### 3. MockLLMAdapter类
适配器模式实现，支持在MockLLM和DeepSeekClient之间显式切换。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `use_mock` | bool | `False` | 是否显式使用MockLLM |
...
```python
# 在生产环境中使用适配器
def get_llm_client_for_production(api_key):
    """获取生产用的LLM客户端"""
...
1. **显式切换**: 通过适配器模式在测试和真实 DeepSeek 之间切换
...
- 在生产环境中保持真实 LLM 失败即报错，不用 Mock 掩盖故障
- 收集和分析LLM调用统计信息
