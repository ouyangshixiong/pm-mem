...
# 使用修复版的模板引擎
from .template_engine_enhanced_fixed import EnhancedTemplateEngine, render_template
...
class EnhancedTemplateManager:
    ...
    def _apply_default_values(self, metadata: TemplateMetadata, context: Dict[str, Any]) -> Dict[str, Any]:
        """应用默认值到上下文"""
        result = context.copy()

        for var_name, var_def in metadata.variables.items():
            if isinstance(var_def, dict):
                # 如果变量不在上下文中但有默认值，则添加默认值
    ...
    def render_template(
        ...
        try:
            ...
            # 应用默认值
            context_with_defaults = self._apply_default_values(template_version.metadata, context)

            # 验证上下文（如果启用）
            if validate:
                validation_result = self.validate_context(
                    template_version.metadata, context_with_defaults
                )
            ...
            # 渲染模板
            result = self.engine.render(template_version.content, context_with_defaults)
